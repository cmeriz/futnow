...
class SearchView extends View {
    _parentElement = document.querySelector('.leagues');
    _searchForm = document.querySelector('.main__form');
    _errorMessage = 'No league was found...';
    _data;
    ...
    getQuery(){
        const query = this._searchForm.querySelector('.search').value.toLowerCase();
        this._clearInput();
        return query;
    }
    ...
    _generateMarkup(){
        ...
        let markup = `<div class="leagues__results">
                        <h2 class="leagues__header">League</h2>
                        <ul class="leagues__list">`;

        markup += this._data.map(league => 
                            `
                                <li class="league" data-league="${league.id}">
                                    <figure>
                                        <img class="league__img img--light" src="${league.logos.light}" alt="${league.name}">
                                        <img class="league__img img--dark" src="${league.logos.dark}" alt="${league.name}">
                                    </figure>
                                    <span class="league__name">${league.name}</span>
                                    <svg class="league__arrow" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
    ...
    _renderError(){
        const markup = `
                <div class="leagues__not-found">
                    ...
                    <h2 class="leagues__not-found__title">${this._errorMessage}</h2>
                </div>
        `;
        this._clear();
