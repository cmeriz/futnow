...
class LeagueView extends View {
    _parentElement = document.querySelector('.league-details');
    _searchedLeagues = document.querySelector('.leagues');
    _standingContainer;
    _stats;

    addHandlerLeague(handler){
        this._searchedLeagues.addEventListener('click', function(e){
            const league = e.target.closest('.league');

            if(!league) return;

            handler(league.dataset.league);
    ...
    _generateMarkup(){
        return `
            <header class="modal-league__header">
                    <figure>
                        <img class="modal-league__img img--light" src="${this._data.logos.light}" alt="${this._data.name}">
                        <img class="modal-league__img img--dark" src="${this._data.logos.dark}" alt="${this._data.name}">
                    </figure>
                    <h2 class="modal-league__name">${this._data.name}</h2>
            </header>
        ` + this._generateStandingMarkup();
    }
    ...
    _generateStandingMarkup(){
        ...
        let markup = `
            <div class="league-standings">
                <div class="modal-league__controls">
                    <select class="modal-league__select season-select">
            `;
        markup += this._data.seasons.map(season => `<option value="${season.year}">${season.year} - ${season.year + 1}</option>`).join('');
        ...
        `
                ...
                <div class="modal-league__table-container league-standings__container">
                `;

        markup += this._generateStandingTable() + `</div></div>`;

        return markup;
    }

    addHandlerSeason(handler){
        document.querySelector('.season-select').addEventListener('change', function(){
            handler(this.value);
        });
    }
    ...
    _generateStandingTable(){
        ...
        markup += this._data.standings.map((standing, index) => {

            let row = 
            `
            <tr>
                <td>
                    <div class="league-table__team">
                        <span class="league-table__team__pos">${index + 1}</span>
                        <figure>
                            <img class="league-table__team__img" src="${standing.team.logos ? standing.team.logos[0].href : './team_logo_placeholder.png'}" alt="${standing.team.name}">
                        </figure>
                        <span class="league-table__team__name">${standing.team.name}</span>
                        <span class="league-table__team__abbr">${standing.team.abbreviation}</span>
                    </div>
                </td>            
            `;
    ...
    _renderError(){
        this._clear();
        this._parentElement.insertAdjacentHTML('afterbegin', this._generateMarkupError(this._errorMessage));
    }

    updateStanding(league){
        this._data = league;
        this._standingContainer.innerHTML = "";
        this._standingContainer.insertAdjacentHTML('afterbegin', this._generateStandingTable());    
    }

    renderErrorStanding(){
        this._standingContainer.innerHTML = "";
        this._standingContainer.insertAdjacentHTML('afterbegin', this._generateMarkupError('Season not found...'));
    }  
    
    renderLoaderStanding(){
        this._standingContainer = document.querySelector('.league-standings__container');
        const loader = `
            <div class="loader-container">
                <span class="loader"></span>
            </div>
        `;
        this._standingContainer.insertAdjacentHTML('afterbegin', loader);
    }
